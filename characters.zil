	...
	(REMOVE
	 <EXECUTE ("AUX" F)
		  ...
		  <COND (<SET F <FIRST-IN-PARTY ,MINAR ,ESHER>>
			 ...
			 <PUT <GETPT .F ,P?PROVISION-COMMANDS>
			      0
			      ,LOOK-AROUND-COMMAND>)>>)
	(ADD
	 <EXECUTE ("AUX" F)
...
<GLOBAL PARTY <TABLE 5 ;4 BERGON PRAXIX ESHER TAG 0>>
...
<ROUTINE MAKE-BUSY (ACT "OPTIONAL" (CMD ,BUSY-COMMAND) "AUX" (OFF 0) TBL)
	 <FSET .ACT ,BUSY>
	 <REPEAT ()
		 <COND (<EQUAL? <GET ,PARTY <SET OFF <+ .OFF 1>>> .ACT>
			...
			<NORMAL-ALL .OFF>
			<RTRUE>)>
		 <COND (<EQUAL? .OFF ,PARTY-MAX> <RFALSE>)>>>
...
<ROUTINE PARTY-ADD (CHR "AUX" NC)
	 ...
	 <SORT-PARTY>
	 <NEW-CHARACTER .CHR>
	 <APPLY <GETP .CHR ,P?ADD>>
	 <RTRUE>>

<ROUTINE PARTY-CHANGE (CHR1 CHR2 "AUX" (CNT <GET ,PARTY 0>))
	 <REPEAT ()
		 <COND (<L? .CNT 1> <RFALSE>)
			...
			<SORT-PARTY>
			<NEW-CHARACTER .CHR2>
			<APPLY <GETP .CHR1 ,P?REMOVE>>
			<APPLY <GETP .CHR2 ,P?ADD>>
			<RTRUE>)>
		 <SET CNT <- .CNT 1>>>>
